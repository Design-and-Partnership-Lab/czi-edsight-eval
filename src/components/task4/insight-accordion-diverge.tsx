...
export default function InsightAccordionDiverged({ header, insights }: InsightAccordionProps) {
    ...
    return (
      <div className="mb-2">
        ...
        {isOpen && (
          <div className="flex justify-between items-center mt-2 bg-white border border-gray-200 rounded-lg mt-2 px-6 py-4 space-y-3">
            {insights.map((text, idx) => (
              <li key={idx} className="list-disc list-inside pl-5 w-full">
                {text}
              </li>
            ))}
          </div>
